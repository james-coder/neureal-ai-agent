"""Agent Research version"""  # pylint: disable=too-many-lines
from collections import OrderedDict
#import multiprocessing as mp
import argparse, time, os, random, socket # keyboard # , talib, bottleneck   # pylint: disable=multiple-imports
...
def main():
    ...
    # _loss-final_lr-snr-99_rwd-prev _out-aio2-mlp _data-same-rnd-N1 _mlp-diff _lat128_lay512-256-64 _val2e5_rep2e8_lEp5_rpP10 _VOar-7
    # _optR _rtnO _prs2 _Oab _lPGv _RfB _train _entropy3 _mae _perO-NR-NT-G-Nrez _rez-rezoR-rezoT-rezoG _mixlog-abs-log1p-Nreparam
    # _obs-tsBoxF-dataBoxI_round _Nexp-Ne9-Nefmp36-Nefmer154-Nefme308-emr-Ndiv _MUimg-entropy-values-policy-Netoe _AC-Nonestep-aing _mem-sort _stepE _cncat
    ...
    parser.add_argument('--gym_make', type=str, default='TraderEnv', help='OpenAI gym environment: [CartPole-v1, LunarLander-v2, Copy, ProcgenChaser,\n' +
                        'ProcgenCaveflyer, Tetris, MontezumaRevenge, MsPacman, Racecar, CartPoleCont, LunarLandCont, BipedalWalker,\n' +
                        'Reacher, Hopper, RacecarZed, PistonBall, TestRnd, DataShkspr, DataMnist, TraderEnv]')
    ...
    parser.add_argument('--value_cont', type=bool, default=True, help='value_cont')
    parser.add_argument('--latent_size', type=int, default=16, help='latent size')
    parser.add_argument('--latent_dist', type=str, default='d', help="'d' = deterministic, 'c' = categorical, 'mx' = continuous(mix-log)")
    ...
    parser.add_argument('--attn_mem_base', type=int, default=4, help='attention mem base')
    ...
    device_type, device, gym_make = args.device_type, args.device_id, args.gym_make
    load_model, save_model = args.load_model, args.save_model
    chkpts, max_episodes, value_cont = args.chkpts, args.max_episodes, args.value_cont
    latent_size, latent_dist = args.latent_size, args.latent_dist
    mixture_multi, net_lstm = args.mixture_multi, args.net_lstm
    aio_max_latents, attn_mem_base = args.aio_max_latents, args.attn_mem_base
    aug_data_step, aug_data_pos, trader = args.aug_data_step, args.aug_data_pos, args.trader
    ...
    if arch == 'TEST':
        pass # testing architechures
    elif arch == 'PG': # Policy Gradient agent
        learn_rates = {'action':4e-6} # PG loss
    elif arch == 'AC': # Actor Critic
        learn_rates = {'action':4e-6, 'value':2e-6} #PG and advantage loss
    elif arch == 'MU': # Combined PG and world model
        learn_rates = {'action':2e-5, 'rep_action':2e-6, 'trans':2e-4, 'rep_trans':2e-6, 'pool':2e-6} # 'act':2e-6

    if gym_make == 'CartPole-v0':
        env_name, max_steps, env_render, env_reconfig, env = 'CartPole', 256, False, True, gym.make('CartPole-v0')
        # env.observation_space.dtype = np.dtype('float64') # (4) float32    ()2 int64    200  195.0
    elif gym_make == 'CartPole-v1':
        env_name, max_steps, env_render, env_reconfig, env = 'CartPole', 512, False, True, gym.make('CartPole-v1')
        # env.observation_space.dtype = np.dtype('float64') # (4) float32    ()2 int64    500  475.0
        ...
        # DuplicatedInput-v0 RepeatCopy-v0 Reverse-v0 ReversedAddition-v0 ReversedAddition3-v0 # ()6 int64    [()2,()2,()5] int64    200  25.0
    elif gym_make == 'ProcgenChaser':
        env_name, max_steps, env_render, env = 'ProcgenChaser', 1024, False, gym.make('procgen-chaser-v0')
        # (64,64,3) uint8    ()15 int64    1000 None
    elif gym_make == 'ProcgenCaveflyer':
        env_name, max_steps, env_render, env = 'ProcgenCaveflyer', 1024, False, gym.make('procgen-caveflyer-v0')
        # (64,64,3) uint8    ()15 int64    1000 None
    elif gym_make == 'Tetris':
        env_name, max_steps, env_render, env = 'Tetris', 22528, False, gym.make('ALE/Tetris-v5')
        # (210,160,3) uint8    ()18 int64    21600 None
    elif gym_make == 'MontezumaRevenge':
        env_name, max_steps, env_render, env = 'MontezumaRevenge', 22528, False, gym.make('MontezumaRevengeNoFrameskip-v4')
        # (210,160,3) uint8    ()18 int64    400000 None
    elif gym_make == 'MsPacman':
        env_name, max_steps, env_render, env = 'MsPacman', 22528, False, gym.make('MsPacmanNoFrameskip-v4')
        # (210,160,3) uint8    ()9 int64    400000 None
    elif gym_make == 'Racecar':
        import pybullet_envs.bullet.racecarGymEnv as racecarGymEnv # pylint: disable=import-outside-toplevel
        env_render = False
        env_name, max_steps, env = 'Racecar', 1024, racecarGymEnv.RacecarGymEnv(isDiscrete=True, renders=env_render)
        # (2) float32    ()9 int64    1000  5.0
    elif gym_make == 'CartPoleCont':
        env_name, max_steps, env_render, env_reconfig, env = 'CartPoleCont', 256, False, True, gym.make('CartPoleContinuousBulletEnv-v0')
        env.observation_space.dtype = np.dtype('float64')
        # (4) float32    (1) float32    200  190.0
    elif gym_make == 'LunarLandCont':
        env_name, max_steps, env_render, env_reconfig, env = 'LunarLandCont', 1024, False, True, gym.make('LunarLanderContinuous-v2')
        # (8) float32    (2) float32    1000  200
    elif gym_make == 'BipedalWalker':
        import envs_local.bipedal_walker as bipedal_walker # pylint: disable=import-outside-toplevel
        env_name, max_steps, env_render, env = 'BipedalWalker', 2048, False, bipedal_walker.BipedalWalker()
    elif gym_make == 'Reacher':
        import pybullet_envs.gym_manipulator_envs as gym_manipulator_envs # pylint: disable=import-outside-toplevel
        env_render = False
        env_name, max_steps, env = 'Reacher', 256, gym_manipulator_envs.ReacherBulletEnv(render=env_render)
        # (9) float32    (2) float32    150  18.0
    elif gym_make == 'Hopper':
        import pybullet_envs.gym_locomotion_envs as gym_locomotion_envs # pylint: disable=import-outside-toplevel
        env_render = False
        env_name, max_steps, env = 'Hopper', 1024, gym_locomotion_envs.HopperBulletEnv(render=env_render)
        # (15) float32    (3) float32    1000  2500.0
    elif gym_make == 'RacecarZed':
        import pybullet_envs.bullet.racecarZEDGymEnv as racecarZEDGymEnv # pylint: disable=import-outside-toplevel
        env_render = False
        env_name, max_steps, env = 'RacecarZed', 1024, racecarZEDGymEnv.RacecarZEDGymEnv(isDiscrete=False, renders=env_render)
        # (10,100,4) uint8    (2) float32    1000  5.0
    elif gym_make == 'PistonBall':
        from pettingzoo.butterfly import pistonball_v6 # pylint: disable=import-outside-toplevel
        env_name, max_steps, env_render, env = 'PistonBall', 1, False, pistonball_v6.env()
    # elif args.gym_make == 'UR5PlayAbs':   #BROKEN, no src.envs.envList!
        # import src.envs.envList as envList
        # env_name, max_steps, env_render, env = 'UR5PlayAbs', 64, False, envList.ExtendedUR5PlayAbsRPY1Obj()
    elif gym_make == 'TestRnd':
        import envs_local.random_env as random_env # pylint: disable=import-outside-toplevel
        env_name, max_steps, env_render, env = 'TestRnd', 64, False, random_env.RandomEnv(True)
    elif gym_make == 'DataShkspr':
        import envs_local.data_env as data_env # pylint: disable=import-outside-toplevel
        env_name, max_steps, env_render, env = 'DataShkspr', 64, False, data_env.DataEnv('shkspr')
    elif gym_make == 'DataMnist':
        import envs_local.data_env as data_env # pylint: disable=import-outside-toplevel
        env_name, max_steps, env_render, env = 'DataMnist', 64, False, data_env.DataEnv('mnist')
    elif gym_make == 'TraderEnv':
        from gym_trader.envs import TraderEnv # pylint: disable=import-outside-toplevel
        tenv = 4
        env_name, max_steps, env_render, env, trader, chart_lim = 'Trader'+str(tenv), 256, False, TraderEnv(agent_id=device, env=tenv), True, 0.0
        extra += f"-rs{env.NUM_EPISODES}-td{int(env.TIMEDELTA_RANGE)}-s{int(env.MAX_EPISODE_TIME/env.TIMEDELTA_RANGE)}-dd{int(env.START_TARGET_BAL)}"
    ...
    with tf.device(f"/device:{device_type}:{device if device_type=='GPU' else 0}"):
        model = GeneralAI(arch, env, trader, env_render, save_model, chkpts, max_episodes, max_steps, learn_rates,
                          value_cont, latent_size, latent_dist, mixture_multi, net_lstm, net_attn, aio_max_latents, attn_mem_base, aug_data_step, aug_data_pos)
        ...
        for layer in model.layers:
            ...
            if (load_model or layer.name == 'M') and tf.io.gfile.exists(model_file):
                ...
                print("LOADED {layer.name} weights from {model_file}")
                loaded_model = True
            if hasattr(layer, 'opt_spec'):
                name_opt = f"-O{layer.opt_spec['type']}{'' if layer.opt_spec['schedule_type'] == '' else '-S' + layer.opt_spec['schedule_type']}"
            else:
                name_opt = ''
        ...
        title = f"{name}    [{device_type}-{tf.keras.backend.floatx()}] {name_arch}\ntime:{util.print_time(total_time)}    steps:{total_steps}    t/s:{step_time:.8f}    ms:{max_steps}"
        title += f"     |     attn:{attn_txt}    al:{aio_max_latents}    am:{attn_mem_base}"
        ...
        if save_model:
            for layer in model.layers:
                model_file = model.model_files[layer.name]
                layer.save_weights(model_file)
                print(f"SAVED {layer.name} weights to {model_file}")
...
class GeneralAI(tf.keras.Model):
    ...
    def __init__(self, arch, env, trader, env_render, save_model, chkpts, max_episodes, max_steps, learn_rates,
        ...
        if latent_dist == 'c': # categorical
            latent_spec.update({'dist_type':'c', 'num_components':0, 'event_shape':(latent_size, latent_size)}) # TODO https://keras.io/examples/generative/vq_vae/
        ...
        # if platform == "win32" and keyboard is not None:
        #     keyboard.add_hotkey('ctrl+alt+k', self.on_stop, suppress=True) # TODO figure out linux/Docker version of this that works
        self.metrics_spec()
    ...
    def PG_learner_onestep(self, inputs, training=True):
        ...
        for step in tf.range(tf.shape(inputs['dones'])[0]):
            ...
            for i in range(self.obs_spec_len):
                obs[i] = inputs['obs'][i][step:step+1]; obs[i].set_shape(self.obs_spec[i]['step_shape'])
            action = [None]*self.action_spec_len
            for i in range(self.action_spec_len):
    ...
    def AC_rep_learner(self, inputs, training=True):
        ...
        for step in tf.range(tf.shape(inputs['dones'])[0]):
            ...
            action = [None]*self.action_spec_len
            for i in range(self.action_spec_len):
                action[i] = inputs['actions'][i][step:step+1]; action[i].set_shape(self.action_spec[i]['step_shape'])
            # returns = inputs['returns'][step:step+1]
            returns_calc = tf.squeeze(tf.cast(returns,self.compute_dtype),axis=-1)
            reward_calc = tf.cast(inputs['rewards'][step],self.compute_dtype)
